...
impl GroupManagementService {
    ...
    async fn from_device(device: Device) -> Result<Self, Error> {
        let urn = "urn:schemas-upnp-org:service:urn:schemas-upnp-org:service:GroupManagement:1:1".parse::<URN>().unwrap();
        let service = device.find_service(&urn)
            .ok_or_else(|| Error::ServiceNotFound("GroupManagementService".to_string()))?;
        Ok(Self{ service: service.clone(), url: device.url().clone() })
    }

    /// Create a new GroupManagementService instance from an IP address.
